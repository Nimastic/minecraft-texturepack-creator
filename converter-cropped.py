...
def calculate_average_color(image):
    ...
    # Exclude transparent pixels from the calculation
    if np_image.shape[2] == 4:  # RGBA image
        alpha_channel = np_image[:, :, 3]
        mask = alpha_channel > 0
        avg_color = np.mean(np_image[mask], axis=0).astype(int)
    else:  # RGB image
